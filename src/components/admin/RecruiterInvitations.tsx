  ...
  return <section className="mh-ui mh-admin" aria-label="Einladungen & Verträge">
    ...
    <div className="mh-stack">
      ...
      {c && <section className="mh-panel mh-stack"><div><p className="mh-kicker">AUSGEWÄHLTER VORGANG</p><h3>{c.profile.name || c.email}</h3><span className="mh-status">{c.revoked_at ? 'Widerrufen' : stateLabels[c.state]}</span></div><p className="text-sm">{c.entry_source === 'website' ? 'Direkte Website-Registrierung' : `Einladungslink gültig bis ${date(c.expires_at)}`} · Gestartet am {date(c.claimed_at)}</p>
        ...
        {['review','approved'].includes(c.state) && !packet && <div className="space-y-2"><label className="block text-sm">Rückfrage an den Headhunter<textarea className="mh-full rounded-md border p-3" value={feedback} onChange={e => setFeedback(e.target.value)}/></label><button className="mh-button" disabled={busy || feedback.trim().length < 10} onClick={() => void action('changes', { feedback })}>Zur Ergänzung öffnen</button></div>}
        ...
        </div><details><summary className="cursor-pointer text-sm">Vertragsvorgang zur Korrektur zurücknehmen</summary><label className="block text-sm">Begründung<textarea className="mh-full rounded-md border p-3" value={feedback} onChange={e => setFeedback(e.target.value)}/></label><button className="mh-button" disabled={busy || feedback.trim().length < 10 || !['prepared','sent'].includes(packet.state)} onClick={() => void action('void', { reason: feedback })}>Vertragsvorgang zurücknehmen</button></details><p className="text-xs text-muted-foreground">Der Vertragsabschluss wird über DocuSign bestätigt. Die Freischaltung der Plattform ist ein eigener Schritt.</p></div>}
      </section>}
      </div></div>
    </div>
  </section>;
