    ...
    <section id="engine" className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
        ...
        <div className="w-[600px] h-[600px] rounded-full border border-border/20 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-spin-reverse">
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 w-3 h-3 bg-emerald/30 rounded-full" />
          <div className="absolute left-0 top-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-emerald/40 rounded-full" />
          <div className="absolute right-0 top-1/2 translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-emerald/35 rounded-full" />
        </div>
        {/* Inner Ring */}
        <div className="w-[400px] h-[400px] rounded-full border border-border/30 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-spin-slow" style={{ animationDuration: '40s' }}>
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-emerald/40 rounded-full" />
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 w-2 h-2 bg-emerald/30 rounded-full" />
        </div>
        {/* Core Ring */}
        ...
        {/* Pulsing Core */}
        <div className="w-[100px] h-[100px] rounded-full bg-emerald/10 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-glow-pulse blur-xl" />
        <div className="w-[60px] h-[60px] rounded-full bg-emerald absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-20 animate-pulse" />
      </div>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid gap-6">
            {layers.map((layer, index) => (
              <div 
                key={index}
                className="group relative p-6 md:p-8 rounded-2xl bg-card border border-border/50 hover:border-emerald/30 hover:shadow-xl hover:shadow-emerald/5 transition-all duration-500"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                {/* Hover Glow Effect */}
                <div className="absolute inset-0 rounded-2xl bg-emerald/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                
                <div className="flex flex-col md:flex-row items-start md:items-center gap-6 relative">
                  {/* Layer Number */}
                  <div className="flex-shrink-0 w-12 h-12 rounded-full bg-muted flex items-center justify-center text-xl font-bold text-muted-foreground group-hover:bg-emerald/10 group-hover:text-emerald transition-all">
                    {index + 1}
                  </div>

                  {/* Icon */}
                  <div className="flex-shrink-0 w-14 h-14 rounded-xl bg-gradient-to-br from-emerald to-emerald-light flex items-center justify-center shadow-lg shadow-emerald/20 group-hover:scale-110 transition-transform duration-300">
