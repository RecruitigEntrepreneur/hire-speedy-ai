  ...
  return MARK_PATHS.map(d => {
    ...
    for (const token of d.match(/[a-zA-Z]|-?\d*\.?\d+/g) ?? []) {
      if (/[a-zA-Z]/.test(token)) {
        if (!/[MmCcLlZz]/.test(token)) throw new Error(`Unerwarteter Pfadbefehl ${token} im Matchunt-Zeichen`);
        absolute = token === token.toUpperCase();
        out.push(token);
        continue;
      }
      ...
      if (pending.length === 2) {
        const [x, y] = pending;
        out.push(absolute ? `${round(x * 0.1 - 400)} ${round(900 - y * 0.1)}` : `${round(x * 0.1)} ${round(-y * 0.1)}`);
        pending = [];
      }
    }
    return out.join(' ');
...
/** SVG-Gruppe des Zeichens, oben links bei (x, y), Breite in Pixeln. */
export function markSvg(x: number, y: number, width: number, color: string): string {
  return `<g transform="translate(${x} ${y}) scale(${width / MARK_BOX.width}) translate(-400 -300)" fill="${color}"><g transform="translate(0 1200) scale(0.1 -0.1)">`
    + MARK_PATHS.map(d => `<path d="${d}"/>`).join('') + '</g></g>';
}
