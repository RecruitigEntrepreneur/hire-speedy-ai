 ...
 *
 * Kunden bekommen denselben Link unter eigenem Schlüssel (client-code.ts): ein
 * Headhunter-Link öffnet nie die Kundenanmeldung und umgekehrt.
 */
...
const storedLinks = (user: User | null | undefined, key: string): StoredLink[] => {
...
export async function issueLoginLink(db: SupabaseClient, userId: string, now = Date.now(), key = LOGIN_LINK_KEY): Promise<string> {
  ...
  const kept = storedLinks(data?.user, key).filter(l => Date.parse(l.expires_at) > now).slice(0, KEEP - 1);
  const { error } = await db.auth.admin.updateUserById(userId, { app_metadata: { [key]: [fresh, ...kept] } });
...
export async function checkLoginLink(db: SupabaseClient, link: unknown, now = Date.now(), key = LOGIN_LINK_KEY): Promise<LoginLinkState> {
  ...
  const match = storedLinks(user, key).find(l => timingSafeEqual(l.hash, hash));
