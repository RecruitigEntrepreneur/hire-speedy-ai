...
BEREITS GESTELLTE FRAGE-IDS (nicht wiederholen): ${asked_ids.join(', ') || '(keine)'}`;
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        tool_choice: { type: 'function', function: { name: 'next_intake_step' } },
      }),
    });

    if (!response.ok) {
      const errText = await response.text();
      console.error('AI-Gateway-Fehler:', response.status, errText.slice(0, 300));
    ...
    try {
      ...
      console.error('Unparsebare AI-Antwort:', String(raw).slice(0, 300));
