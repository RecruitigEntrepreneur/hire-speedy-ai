...
import { must, dbError, caseById, patchCase, envelopeById, patchEnvelope, signatureConfig, providerRequest, sha256, syncEnvelope, workflowFailure, type RecruiterEnvelope } from '../_shared/recruiter-onboarding-service.ts';
...
serve(async req => {
  ...
  try {
    ...
    if (body.action === 'mail') {
      ...
      const url = `${getPublicAppUrl()}/recruiter/invitation#${body.token}`;
      const note = String(body.message ?? '').trim().slice(0, 1200);
      const result = await sendIntakeMail(db, {
        to: c.email, subject: 'Ihre Einladung zum Matchunt Recruiter-Netzwerk',
        template: 'recruiter_onboarding_invitation', replyTo: admin.email,
