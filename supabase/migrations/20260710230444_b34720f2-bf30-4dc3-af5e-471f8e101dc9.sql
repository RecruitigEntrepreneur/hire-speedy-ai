...
CREATE POLICY "Client admins can delete jobs" ON public.jobs
  FOR DELETE USING (
    ...
    OR (client_id = auth.uid() AND public.can_edit_job(id))
  );
...
-- ----------------------------------------------------------------------------
-- 9) Client-Views auf Team-Scoping umstellen
-- ----------------------------------------------------------------------------
