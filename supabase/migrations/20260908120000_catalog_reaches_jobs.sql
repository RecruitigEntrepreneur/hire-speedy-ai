-- Der Fragenkatalog erreicht die Stelle
--
-- BEFUND (08.09.2026): accept_intake_draft hatte eine FEST VERDRAHTETE
-- Spaltenliste. draftToJobRow durfte in _job schreiben, was es wollte -- alles,
-- was nicht in dieser Liste stand, wurde still verworfen. Von 37 Zielspalten
-- des Fragenkatalogs kamen SECHS an. Monatsgehaelter, Betriebsrat, Alltag,
-- Kultur, Karriere, Muss-Kriterien, Teamgroesse, Kernzeiten: alles erhoben,
-- alles im Entwurf gespeichert, nichts in der Stelle.
--
-- Diese Migration ist absichtlich selbsttragend: die drei Spalten aus
-- 20260905090000 werden hier noch einmal mit IF NOT EXISTS angelegt, damit sie
-- auch dann existieren, wenn jene Migration noch nicht angewandt wurde. Sonst
-- scheitert der INSERT an einer fehlenden Spalte und die Annahme bricht ganz.

BEGIN;

-- Selbsttragend: dieselben drei Spalten wie in 20260905090000, idempotent.
...
BEGIN
  ...
  INSERT INTO public.jobs (
    ...
    company_size_band, candidates_dropped_reason, team_size,
...
COMMENT ON FUNCTION public.accept_intake_draft(uuid, uuid, uuid, uuid, jsonb, uuid) IS
  ...
  'traegt der INSERT auch die 31 Spalten des Fragenkatalogs. Vorher hatte die '
