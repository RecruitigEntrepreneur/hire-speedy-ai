-- ============================================================================
-- "Verhandelbar" kommt beim Headhunter an
-- ============================================================================
-- BEFUND (09.09.2026): Der Kunde stuft jedes Kriterium in drei Stufen ein --
-- unverzichtbar, verhandelbar, lernbar. Zwei davon erreichen die Stelle seit
-- 20260908120000. Die dritte nicht: `nice_to_have_criteria` gibt es als Spalte
-- in `jobs` und in recruiter_jobs_view, aber accept_intake_draft nannte sie
-- nicht in ihrer INSERT-Liste -- und die Funktion verwirft still, was nicht
-- in der Liste steht.
--
-- Damit fehlte dem Headhunter genau die mittlere Stufe: was NICHT Pflicht ist,
-- aber auch nicht nachschulbar -- der Verhandlungsspielraum. Ohne sie liest
-- sich ein Profil als "Pflicht oder egal".
--
-- Diese Migration schreibt die Funktion mit derselben Fassung wie
-- 20260908120000 neu, ergaenzt um diese eine Spalte. Keine Tabellen-, keine
-- View-Aenderung: die Spalte existiert seit Januar, die View fuehrt sie seit
-- 20260905090000.
-- ============================================================================
...
COMMENT ON FUNCTION public.accept_intake_draft(uuid, uuid, uuid, uuid, jsonb, uuid) IS
  ...
  'traegt der INSERT auch die 33 Spalten des Fragenkatalogs. Vorher hatte die '
