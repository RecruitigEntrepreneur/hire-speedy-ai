-- ============================================================================
-- Die Mitarbeiterzahl, nicht nur ihr Band
-- ============================================================================
-- BEFUND (09.09.2026, Testaufnahme "Senior Platform Engineer"): Die Anzeige
-- sagte "rund 340 Mitarbeitende". sizeBand() machte daraus "250-1.000" und
-- die 340 war weg -- eine Spanne mit Faktor vier. Fuer einen Headhunter ist
-- das der Unterschied zwischen einem 260-Mann-Betrieb mit kurzen Wegen und
-- einem 950-Mann-Haus mit Konzernstrukturen; er argumentiert im
-- Kandidatengespraech voellig anders.
--
-- Schlimmer war die Vorbelegung: unter dem Band stand "aus der Anzeige --
-- bitte pruefen". Der Kunde bestaetigte damit eine Spanne, die er nie genannt
-- hatte, und danach stand sie als seine Angabe da.
--
-- WARUM DAS BAND TROTZDEM BLEIBT: company_size_band ist in
-- recruiter_jobs_view ungated und speist getDisplayCompanyName -- die anonyme
-- Firmenbeschreibung vor dem Reveal. "340 Mitarbeitende, Prozessindustrie,
-- Raum Stuttgart" ist fuer jeden, der den Markt kennt, praktisch eine
-- Adresse. Die genaue Zahl steht deshalb hinter demselben Gate wie
-- company_name und description: vorher liest der Recruiter die Klasse, nach
-- dem Reveal die Zahl.
--
-- Das Band wird ab jetzt aus der Zahl abgeleitet, nicht mehr daneben gefragt.
-- ============================================================================

BEGIN;

ALTER TABLE public.jobs
  ADD COLUMN IF NOT EXISTS company_headcount integer;

COMMENT ON COLUMN public.jobs.company_headcount IS
  'Genaue Mitarbeiterzahl des Kunden. REVEAL-GESPERRT: zusammen mit Branche '
  'und Region identifiziert sie das Unternehmen. Der ungegatete Ersatz ist '
  'company_size_band, das aus dieser Zahl abgeleitet wird.';

-- ---------------------------------------------------------------------------
-- recruiter_jobs_view neu, mit der Zahl hinter dem Gate
-- ---------------------------------------------------------------------------
-- Basis ist unveraendert 20260908110000; company_size_band bleibt, wo es war.
...
-- ---------------------------------------------------------------------------
-- Dieselbe Fassung wie 20260909110000, ergaenzt um company_headcount. Ohne
-- diesen Teil kaeme der Wert an der RPC an und wuerde still verworfen -- die
-- Funktion hat eine fest verdrahtete Spaltenliste.
...
BEGIN
  ...
  INSERT INTO public.jobs (
    ...
    contract_sent_digitally, must_have_criteria, trainable_skills,
...
COMMENT ON FUNCTION public.accept_intake_draft(uuid, uuid, uuid, uuid, jsonb, uuid) IS
  ...
  'traegt der INSERT auch die 34 Spalten des Fragenkatalogs. Vorher hatte die '
