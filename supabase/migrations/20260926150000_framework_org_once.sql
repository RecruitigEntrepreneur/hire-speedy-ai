-- ============================================================================
-- Rahmenvertrag: die Firma darf nach der Unterschrift einmal nachgetragen werden
-- ============================================================================
-- Befund (Live, 25./26.09.2026, Kanna Medics): Der Rahmenvertrag entsteht beim
-- Absenden der Aufnahme, die Firma (organizations) erst bei der Annahme -- nach
-- Unterschrift und Gegenzeichnung. framework_guard (c) verbot nach der
-- Kundenunterschrift JEDE Änderung an organization_id, auch leer -> Firma.
-- Deshalb blieb RV-2026-001002 ohne Firma: Die Admin-Seite zeigte „kein
-- Rahmenvertrag“, und das Nachtragen bei der Annahme (framework-link.ts,
-- 0d5c89e) scheiterte still.
--
-- organization_id ist keine Vertragsangabe: Der unterschriebene Text steht im
-- Snapshot, die Firma dort in client.* -- geschützt bleibt beides. Erlaubt wird
-- nur das einmalige Setzen; eine gesetzte Firma lässt sich weiterhin nicht
-- ändern. Die Eindeutigkeit (ein wirksamer Rahmenvertrag je Firma) sichert
-- weiter der Index client_framework_one_active_idx.
--
-- Sonst unverändert aus 20260902100100_framework_and_assignment.sql.
-- ============================================================================
...
BEGIN
  ...
  IF OLD.customer_signed_at IS NOT NULL THEN
    ...
    -- Die Firma darf EINMAL nachgetragen werden (leer -> Firma), weil sie
    -- erst bei der Annahme entsteht; eine gesetzte Firma bleibt, wie sie ist.
