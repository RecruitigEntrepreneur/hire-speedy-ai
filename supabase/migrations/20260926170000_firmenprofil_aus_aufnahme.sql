...
--
-- Befund (Luca Bartosch, Kanna Medics): Die Annahme legte kein Firmenprofil
-- an. Firmierung, Anschrift, Register und USt-IdNr. lagen nur in der Aufnahme;
-- der Start-Kasten im Dashboard meldete "Firmendaten fehlt".
--
-- 1. Spalten für das Arbeitgeberprofil, mit denen profileQuality schon rechnet
--    (Arbeitgeberargumente, Ziel-/No-Go-Firmen), und die Herkunft der aus der
--    Aufnahme übernommenen Felder.
-- 2. Einmal nachtragen: für jeden Kunden mit angenommener Aufnahme das Profil
--    aus seiner jüngsten Aufnahme füllen -- NUR leere Felder, nie überschreiben.
--    Ab jetzt erledigt das intake-admin bei der Annahme (_shared/firmenprofil.ts).
-- ============================================================================
...
WITH quelle AS (
  SELECT DISTINCT ON (j.client_id)
    ...
    -- Firmierung: ausdrücklich angegeben, sonst der Firmenname, wenn er eine
    -- Rechtsform trägt (dieselbe Regel wie _shared/firma-pflicht.ts).
    COALESCE(
...
ON CONFLICT (user_id) DO UPDATE SET
  -- Nur leere Felder füllen. Alle Ausdrücke sehen die ALTEN Werte von p.
